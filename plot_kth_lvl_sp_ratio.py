...
def main():
    ...
    for i, j in enumerate(j_arr):  # Iterate over j values with an index
        ...
        plt.axhline(y=r_avg_goe, linestyle='-', color=color, alpha=0.8, 
                    label=rf'$\left\langle r\right\rangle_{{\text{{GOE Num}}}}, j={j}$')
        plt.axhline(y=r_avg_poi, linestyle='--', color=color, alpha=0.8, 
                    label=rf'$\left\langle r\right\rangle_{{\text{{Poi num}}}}, j={j}$')
    ...
    plt.savefig(f'plots/Dicke_Level_spacing_ratio_M={M}_ω={ω}_ω0={ω0}_gc={gc}_α={α}_k={k}.png')
    plt.show()
