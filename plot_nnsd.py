...
def main():
    ...
    for g_ind, g in enumerate(g_arr):
        ...
        if unfl_proc == "local":
            eigval_sp = eigval_sp_fun(eigvals, α, v)
        elif unfl_proc == "poly":
    ...
    if not os.path.exists("plots"):
        os.mkdir("plots")
    if unfl_proc == "local":
        plt.savefig(f'plots/Dicke_NNSD_j={j}_M={M}_α={α}_v={v}.jpg')
    elif unfl_proc == "poly":
        plt.savefig(f'plots/Dicke_NNSD_poly_j={j}_M={M}_α={α}_deg={deg}.jpg')
    elif unfl_proc == None:
        plt.savefig(f'plots/Dicke_NNSD_poly_j={j}_M={M}_α={α}.jpg')
